...
export const ordersRouter = createTRPCRouter({
  list: publicProcedure.input(ListOrderSchema).query(async ({ ctx, input }) => {
    ...
    if (input.dateFrom) {
      dateClauses.push(
        gte(orders.orderDate, input.dateFrom.toISOString().slice(0, 10))
      );
    }
    if (input.dateTo) {
      dateClauses.push(
        lte(orders.orderDate, input.dateTo.toISOString().slice(0, 10))
      );
    }
    const whereClause =
      dateClauses.length > 0 ? and(...dateClauses) : undefined;
    ...
    if (input.cursor !== undefined) {
      const items = await ctx.db
        ...
        .where(
          whereClause
            ? and(whereClause, gt(orders.id, input.cursor))
            : gt(orders.id, input.cursor)
        )
        .orderBy(input.order === "asc" ? asc(orders.id) : desc(orders.id))
        .limit(input.limit + 1);
  ...
  updateItem: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return await ctx.db.transaction(async (tx) => {
        ...
        if (order.status !== "pending") {
          throw new Error(
            "Cannot update items in a completed or cancelled order"
          );
        }
