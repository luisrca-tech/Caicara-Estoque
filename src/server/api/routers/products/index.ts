...
import { desc, eq, gt, asc, or, and, ilike, count } from "drizzle-orm";
import { z } from "zod";
...
export const productsRouter = createTRPCRouter({
  list: publicProcedure.input(ListProductsSchema).query(async ({ ctx, input }) => {
  ...
  adjustQuantity: publicProcedure.input(z.object({
    id: z.number().int().positive().min(1, { message: "ID must be greater than 0" }),
    delta: z.number().int(),
  })).mutation(async ({ ctx, input }) => {
